...
const CATEGORIES = ['burgers', 'chicken', 'breakfast', 'sides', 'drinks'] as const
...
const ALL_ALLERGENS = ['dairy', 'wheat', 'soy', 'eggs', 'fish', 'peanuts', 'treeNuts', 'shellfish']
...
function applyChanges(records: JdeRecord[], changePct: number): JdeRecord[] {
  return records.map((record, i) => {
    ...
    return {...record, basePrice: Math.round((record.basePrice + 0.5) * 100) / 100}
  })
}
...
function mutationVolumeLabel(monthlyMutations: number): {label: string; color: string} {
  if (monthlyMutations < 500_000) return {label: 'Low — well within any plan', color: 'text-green-600'}
  if (monthlyMutations < 5_000_000) return {label: 'Moderate — monitor usage', color: 'text-yellow-600'}
  return {label: 'High — enterprise discussion needed', color: 'text-red-600'}
}
...
export default function SyncDemoPage() {
  ...
  const baseRecords = Array.from({length: recordCount}, (_, i) => generateJdeRecord(i))
  ...
  async function handleSeed() {
    ...
    try {
      ...
      const result = await callApi({action: 'seed', records: baseRecords, dryRun: false})
  ...
  async function handleSync() {
    ...
    try {
      if (mode === 'diff') {
        const result = await callApi({action: 'sync', records: recordsWithChanges, naive: false, dryRun})
        setDiffResult(result)
      } else {
        const result = await callApi({action: 'sync', records: recordsWithChanges, naive: true, dryRun})
  ...
  async function handleCompare() {
    ...
    try {
      const [d, n] = await Promise.all([
        callApi({action: 'sync', records: recordsWithChanges, naive: false, dryRun}),
        callApi({action: 'sync', records: recordsWithChanges, naive: true, dryRun}),
      ])
      setDiffResult(d)
      setNaiveResult(n)
      setPhase('synced')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 text-sm">🔄 JDE Sync Architecture Demo</p>
            </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        {/* Page heading */}
        <div>
          <h2 className="text-3xl font-black text-gray-900 mb-2">🔄 JDE Sync Architecture Demo</h2>
          <p className="text-gray-600 text-lg">
            How do you sync{' '}
            <strong>60,000 JDE product records into Sanity 6×/day</strong> without blowing your API
            quota?
          </p>
          <p className="text-gray-500 mt-2">
            Three techniques — diff-before-write, transaction batching, and CDN reads — combine to
            keep the call profile dramatically lower than a naïve approach.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <h3 className="font-bold text-gray-900 mb-5 text-lg">Architecture: Sync Data Flow</h3>
          ...
          <div className="overflow-x-auto pb-2">
            <div className="flex items-stretch gap-0 min-w-max text-sm">
              ...
              <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-4 text-center w-36">
                ...
                <div className="text-xs text-yellow-600 mt-1">Read current state</div>
                <div className="mt-2 bg-yellow-100 rounded px-2 py-0.5">
                  <span className="text-xs font-medium text-yellow-700">Skip if unchanged</span>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border-2 border-blue-400 rounded-lg p-4 text-center w-36">
                ...
                <div className="mt-2 bg-blue-100 rounded px-2 py-0.5">
                  <span className="text-xs font-medium text-blue-700">= 1 API call</span>
                </div>
              </div>
              ...
              <div className="bg-green-50 border-2 border-green-400 rounded-lg p-4 text-center w-36">
                ...
                <div className="text-xs text-green-600 mt-1">productcatalog</div>
                <div className="mt-2 bg-green-100 rounded px-2 py-0.5">
                  <span className="text-xs font-medium text-green-700">Source of truth</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg px-4 py-2 text-center">
              ...
              <span className="text-xs text-purple-500 ml-2">
                → served from edge,{' '}
                <strong className="text-purple-700">much higher quota</strong>, no impact on
                write quota
              </span>
            </div>
            <div className="flex-1 border-t-2 border-dashed border-gray-300" />
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-yellow-50 rounded-lg p-3">
              <p className="text-xs font-semibold text-yellow-700">🔍 Diff reads via CDN</p>
              <p className="text-xs text-yellow-600 mt-1">
                Reads current state via CDN — much higher quota than the write API, and doesn&apos;t
                touch your mutation quota at all. Syncing every few hours? CDN staleness is
                seconds — completely irrelevant.
              </p>
            </div>
            <div className="bg-blue-50 rounded-lg p-3">
              <p className="text-xs font-semibold text-blue-700">📦 Transaction batching</p>
              <p className="text-xs text-blue-600 mt-1">
                All mutations for a sync cycle are bundled into one transaction. N changed
                documents = 1 write API call. Atomic commit with rollback on error.
              </p>
            </div>
            <div className="bg-purple-50 rounded-lg p-3">
              <p className="text-xs font-semibold text-purple-700">📡 CDN reads — high quota, no mutation cost</p>
              <p className="text-xs text-purple-600 mt-1">
                Customers, kiosks, and mobile apps read from Sanity&apos;s CDN edge. CDN reads have
                a much higher quota than the write API and don&apos;t touch your mutation quota at
                all.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <h3 className="font-bold text-gray-900 text-lg mb-1">⚡ Live Sync Simulator</h3>
          <p className="text-sm text-gray-500 mb-1">
            Two-phase demo against the{' '}
            <span className="font-mono bg-gray-100 px-1 rounded">jdedemo</span> sandbox dataset.
          </p>

          {/* Phase stepper */}
          <div className="flex items-center gap-2 mb-6 text-xs font-semibold mt-3">
            <div className={`px-3 py-1 rounded-full border-2 transition-colors ${phase === 'idle' ? 'bg-bh-dark text-white border-bh-dark' : 'border-gray-300 text-gray-400'}`}>
              1 · Seed dataset
            </div>
            <div className="text-gray-300">──</div>
            <div className={`px-3 py-1 rounded-full border-2 transition-colors ${phase === 'seeded' ? 'bg-bh-dark text-white border-bh-dark' : phase === 'synced' ? 'border-green-400 text-green-600' : 'border-gray-300 text-gray-400'}`}>
              2 · Run sync cycle
            </div>
            <div className="text-gray-300">──</div>
            <div className={`px-3 py-1 rounded-full border-2 transition-colors ${phase === 'synced' ? 'bg-green-600 text-white border-green-600' : 'border-gray-300 text-gray-400'}`}>
              3 · See results
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <input
                ...
                step={5}
                value={recordCount}
                onChange={(e) => { setRecordCount(Number(e.target.value)); setPhase('idle'); setSeedResult(null); setDiffResult(null); setNaiveResult(null) }}
                disabled={isRunning}
            ...
            <div className={phase === 'idle' ? 'opacity-40 pointer-events-none' : ''}>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
          ...
          <div className={`mb-6 ${phase === 'idle' ? 'opacity-40 pointer-events-none' : ''}`}>
            <p className="text-sm font-semibold text-gray-700 mb-2">Sync strategy:</p>
            ...
            <label className="flex items-center gap-3 cursor-pointer select-none w-fit">
              ...
              <div>
                <span className="text-sm font-semibold text-gray-700">
                  {dryRun ? '🔵 Dry run sync — no writes committed' : '🟠 Live sync — writes to jdedemo dataset'}
                </span>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            >
              {runningOp === 'seed' ? (
                <span className="flex items-center gap-2"><span className="animate-spin">⏳</span> Seeding…</span>
              ) : phase === 'idle' ? (
          ...
          {isRunning && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700 animate-pulse mb-4">
              {runningOp === 'seed' && `Writing ${recordCount} records to jdedemo…`}
              {runningOp === 'sync' && `Reading current state from Sanity, diffing ${recordCount} records, committing transaction…`}
          ...
          {seedResult && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4 text-sm">
              ...
              <p className="text-gray-500 text-xs">
                ...
                <strong>1 write transaction</strong>.{' '}
                Now adjust the sliders and run a sync cycle to see diff-check in action.
              </p>
            </div>
          )}
            ...
            >
              {diffResult && (
                <ResultCard
                  ...
                  naiveMonthlyMutations={naiveResult
        ...
        <div className="bg-bh-cream border border-bh-gold rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              {
                icon: '📡',
                title: 'CDN reads have a much higher quota — and don\'t touch mutation quota',
          ...
          <div className="mt-5 border-t border-bh-gold/40 pt-4">
            <blockquote className="border-l-4 border-bh-gold pl-4 italic text-gray-600 text-sm">
              &ldquo;We sync 60K items from JDE 6 times a day. We can&apos;t afford to blow our API
              quota on records that didn&apos;t change.&rdquo;
            </blockquote>
            ...
            <div className="mt-3 bg-green-50 border border-green-200 rounded-lg p-3 ml-5">
              ...
              <p className="text-xs text-green-600 mt-1">
                Diff-before-write + transaction batching + CDN reads = a very efficient API call
                profile even at 60K documents syncing 6×/day. The simulator above runs this for
                real.
              </p>
            </div>
          </div>
...
function ResultCard({result, label, borderColor, headerBg, naiveMonthlyMutations}: ResultCardProps) {
  ...
  const savingsPct = naiveMonthlyMutations > 0
    ? Math.round((mutationSavings / naiveMonthlyMutations) * 100)
    : 0

  return (
    <div className={`border-l-4 ${borderColor} bg-white rounded-xl shadow overflow-hidden`}>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <Stat value={result.totalRecords} label="Records" sublabel="processed" />
          <Stat
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
          ...
          <p className="text-2xl font-black text-orange-900">
            ...
            <span className="text-sm font-normal text-orange-600 ml-1">mutations/mo</span>
          </p>
          <p className={`text-xs font-semibold mt-0.5 ${volumeLabel.color}`}>{volumeLabel.label}</p>

          {mutationSavings > 0 && (
            <div className="mt-2 bg-green-100 border border-green-200 rounded p-2">
              <p className="text-xs font-bold text-green-700">
                💰 {mutationSavings.toLocaleString()} fewer mutations/mo vs. naive
              </p>
              <p className="text-xs text-green-600">{savingsPct}% reduction in write volume</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-3 space-y-1.5">
          <p className="text-xs font-semibold text-gray-500 mb-2">API calls this sync cycle:</p>
          ...
          {result.mode === 'naive' && (
            <ApiCallRow icon="📖" label="Read calls" value={0} note="none — naive skips reads" />
          )}
          <ApiCallRow
            icon="✍️"
          ...
          <div className="border-t border-gray-200 pt-1.5 flex justify-between items-center">
            <span className="text-xs font-bold text-gray-500">Quota-counted calls:</span>
            <span className="text-sm font-black text-gray-700">{result.writeTransactions}</span>
          </div>
        </div>
      </div>
    </div>
...
function Stat({
  ...
  return (
    <div className={`rounded-lg p-2 ${highlight ? 'bg-orange-50' : 'bg-gray-50'}`}>
      <p className={`text-2xl font-black ${highlight ? 'text-orange-600' : 'text-gray-800'}`}>
        {value}
      </p>
...
function ApiCallRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className="text-sm font-bold text-gray-800 flex-shrink-0">{value}</span>
    </div>
  )
}
